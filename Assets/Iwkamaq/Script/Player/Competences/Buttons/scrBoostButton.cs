...
public class scrBoostButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    bool mouseOn;
    public Text text;
    ...
    void Update()
    {
        ...
        if (mouseOn && Input.GetAxis("Slot One Input") != 0 && scrExperienceManager.ExperienceManager.baseBoostUnlocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInFirstSlot(scrCompetenceManager.CompetenceManager.boostName);
        }

        if (mouseOn && Input.GetAxis("Slot Two Input") != 0 && scrExperienceManager.ExperienceManager.baseBoostUnlocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInSecondSlot(scrCompetenceManager.CompetenceManager.boostName);
        }

        if (mouseOn && Input.GetAxis("Slot Three Input") != 0 && scrExperienceManager.ExperienceManager.baseBoostUnlocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInThirdSlot(scrCompetenceManager.CompetenceManager.boostName);
        }
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        if (scrExperienceManager.ExperienceManager.baseBoostUnlocked)
            text.text = "R To Use";
        else
    ...
    void FeedbackUpdate()
    {
        ...
        if (scrExperienceManager.ExperienceManager.currentExperience >= scrExperienceManager.ExperienceManager.firstLevelCompCost && !scrExperienceManager.ExperienceManager.baseBoostUnlocked)
        {
            //Shake du bouton
            if (mouseOn)
            {
        ...
        if (!scrExperienceManager.ExperienceManager.baseBoostUnlocked && scrExperienceManager.ExperienceManager.currentExperience < scrExperienceManager.ExperienceManager.firstLevelCompCost)
        {
        ...
        //débloqué, pas équipé
        if (scrExperienceManager.ExperienceManager.baseBoostUnlocked && !scrCompetenceManager.CompetenceManager.boostEquiped)
        {
    ...
    void ParticlesUpdate()
    {
        ...
        if ((scrCompetenceManager.CompetenceManager.boostEquiped
    || (!scrExperienceManager.ExperienceManager.baseBoostUnlocked
    ...
    && !unlockableParticles.isEmitting)
        ...
        if ((scrExperienceManager.ExperienceManager.baseBoostUnlocked
            ...
            || (CurrentColor.r <= 0.17f && !mouseOn))
            && unlockableParticles.isEmitting
            && !scrCompetenceManager.CompetenceManager.boostEquiped)
        {
        ...
        if (mouseOn && !scrExperienceManager.ExperienceManager.baseBoostUnlocked)
            unlockableParticlesMain.simulationSpeed = 5;
        else
            unlockableParticlesMain.simulationSpeed = 1;

        if (scrCompetenceManager.CompetenceManager.boostEquiped && !equipedParticles1.isPlaying)
        {
