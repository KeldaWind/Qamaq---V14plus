...
public class scrTotemButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Update()
    {
        ...
        if (mouseOn && Input.GetAxis("Slot One Input") != 0 && scrExperienceManager.ExperienceManager.totemUnclocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInFirstSlot(scrCompetenceManager.CompetenceManager.totemName);
        }

        if (mouseOn && Input.GetAxis("Slot Two Input") != 0 && scrExperienceManager.ExperienceManager.totemUnclocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInSecondSlot(scrCompetenceManager.CompetenceManager.totemName);
        }

        if (mouseOn && Input.GetAxis("Slot Three Input") != 0 && scrExperienceManager.ExperienceManager.totemUnclocked)
        {
            scrCompetenceManager.CompetenceManager.EquipInThirdSlot(scrCompetenceManager.CompetenceManager.totemName);
        }
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        if (scrExperienceManager.ExperienceManager.totemUnclocked)
            text.text = "F To Use";
        else
    ...
    void FeedbackUpdate()
    {
        ...
        if (scrExperienceManager.ExperienceManager.currentExperience >= scrExperienceManager.ExperienceManager.firstLevelCompCost && !scrExperienceManager.ExperienceManager.totemUnclocked)
        {
            //Shake du bouton
            if (mouseOn)
            {
        ...
        if (!scrExperienceManager.ExperienceManager.totemUnclocked && scrExperienceManager.ExperienceManager.currentExperience < scrExperienceManager.ExperienceManager.firstLevelCompCost)
        {
        ...
        if (scrExperienceManager.ExperienceManager.totemUnclocked && !scrCompetenceManager.CompetenceManager.totemEquiped)
        {
    ...
    {
        //if (!scrExperienceManager.ExperienceManager.totemUnclocked && scrExperienceManager.ExperienceManager.currentExperience >= scrExperienceManager.ExperienceManager.firstLevelCompCost && mouseOn)

        //débloqué, pas équipé
        /*if (scrExperienceManager.ExperienceManager.totemUnclocked && !unlockableParticles.isEmitting)
        {
        ...
        if ((scrCompetenceManager.CompetenceManager.totemEquiped
            || (!scrExperienceManager.ExperienceManager.totemUnclocked
        ...
        if ((scrExperienceManager.ExperienceManager.totemUnclocked
            ...
            || (CurrentColor.r <= 0.17f && !mouseOn))
            && unlockableParticles.isEmitting
            && !scrCompetenceManager.CompetenceManager.totemEquiped)
        {
            unlockableParticles.Stop();
        }
        ...
        //acceleration pour une competence achetable et souris dessus
        if (mouseOn && !scrExperienceManager.ExperienceManager.totemUnclocked)
            unlockableParticlesMain.simulationSpeed = 5;
        else
            unlockableParticlesMain.simulationSpeed = 1;

        if (scrCompetenceManager.CompetenceManager.totemEquiped && !equipedParticles1.isPlaying)
        {
